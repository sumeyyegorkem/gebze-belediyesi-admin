...
if (isset($_GET['durum_guncelle'], $_GET['id'])) {
    ...
    if (in_array($yeniDurum, $izinliDurumlar, true) && $guncellenecekId > 0) {
        ...
        $kayitStmt = $pdo->prepare("SELECT ad_soyad FROM emlak_sorgu_talepleri WHERE id = :id");
        $kayitStmt->execute(['id' => $guncellenecekId]);
        $kayitAdi = $kayitStmt->fetchColumn();

        $guncelle = $pdo->prepare("UPDATE emlak_sorgu_talepleri SET durum = :durum WHERE id = :id");
        $guncelle->execute(['durum' => $yeniDurum, 'id' => $guncellenecekId]);

        if ($kayitAdi !== false) {
            islemKaydet($pdo, 'Emlak Sorgu Talebi Durumunu ' . ($durumEtiketleriLog[$yeniDurum] ?? $yeniDurum) . ' Yaptı', $kayitAdi);
        }
    }
    header('Location: emlak-talepler-yonet.php?basarili=1');
    exit;
}
...
if (isset($_GET['sil'])) {
    ...
    if ($silinecekId > 0) {
        $silinecekStmt = $pdo->prepare("SELECT ad_soyad FROM emlak_sorgu_talepleri WHERE id = :id");
        $silinecekStmt->execute(['id' => $silinecekId]);
        $silinecekAdi = $silinecekStmt->fetchColumn();

        $sil = $pdo->prepare("DELETE FROM emlak_sorgu_talepleri WHERE id = :id");
        $sil->execute(['id' => $silinecekId]);

        if ($silinecekAdi !== false) {
            islemKaydet($pdo, 'Emlak Sorgu Talebi Sildi', $silinecekAdi);
        }
    }
    header('Location: emlak-talepler-yonet.php?basarili=1');
    exit;
...
$arama = trim($_GET['q'] ?? '');
if ($arama !== '') {
    $stmt = $pdo->prepare("SELECT * FROM emlak_sorgu_talepleri WHERE ad_soyad LIKE :q1 OR mahalle LIKE :q2 OR ada_parsel LIKE :q3 ORDER BY gonderim_tarihi DESC");
    $stmt->execute(['q1' => '%' . $arama . '%', 'q2' => '%' . $arama . '%', 'q3' => '%' . $arama . '%']);
    $talepler = $stmt->fetchAll();
} else {
    $talepler = $pdo->query("SELECT * FROM emlak_sorgu_talepleri ORDER BY gonderim_tarihi DESC")->fetchAll();
}
...
$sayfaBasligi = 'Emlak Sorgu Talepleri';
$aktifMenu = 'emlak-talepler';
include 'includes/admin-baslangic.php';
?>

<h4 class="fw-bold mb-3">Emlak Sorgu Talepleri</h4>
            ...
            <tbody>
                ...
                <?php foreach ($talepler as $t): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($t['ada_parsel']); ?></td>
