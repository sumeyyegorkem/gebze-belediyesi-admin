...
if (isset($_GET['durum_guncelle'], $_GET['id'])) {
    ...
    if (in_array($yeniDurum, $izinliDurumlar, true) && $guncellenecekId > 0) {
        ...
        $kayitStmt = $pdo->prepare("SELECT gelin_damat_adi FROM nikah_randevu_talepleri WHERE id = :id");
        $kayitStmt->execute(['id' => $guncellenecekId]);
        $kayitAdi = $kayitStmt->fetchColumn();

        $guncelle = $pdo->prepare("UPDATE nikah_randevu_talepleri SET durum = :durum WHERE id = :id");
        $guncelle->execute(['durum' => $yeniDurum, 'id' => $guncellenecekId]);

        if ($kayitAdi !== false) {
            islemKaydet($pdo, 'Nikah Randevu Talebi Durumunu ' . ($durumEtiketleriLog[$yeniDurum] ?? $yeniDurum) . ' Yaptı', $kayitAdi);
        }
    }
    header('Location: nikah-talepler-yonet.php?basarili=1');
    exit;
}
...
if (isset($_GET['sil'])) {
    ...
    if ($silinecekId > 0) {
        $silinecekStmt = $pdo->prepare("SELECT gelin_damat_adi FROM nikah_randevu_talepleri WHERE id = :id");
        $silinecekStmt->execute(['id' => $silinecekId]);
        $silinecekAdi = $silinecekStmt->fetchColumn();

        $sil = $pdo->prepare("DELETE FROM nikah_randevu_talepleri WHERE id = :id");
        $sil->execute(['id' => $silinecekId]);

        if ($silinecekAdi !== false) {
            islemKaydet($pdo, 'Nikah Randevu Talebi Sildi', $silinecekAdi);
        }
    }
    header('Location: nikah-talepler-yonet.php?basarili=1');
    exit;
...
$arama = trim($_GET['q'] ?? '');
if ($arama !== '') {
    $stmt = $pdo->prepare("SELECT * FROM nikah_randevu_talepleri WHERE gelin_damat_adi LIKE :q1 OR esinin_adi LIKE :q2 ORDER BY gonderim_tarihi DESC");
    $stmt->execute(['q1' => '%' . $arama . '%', 'q2' => '%' . $arama . '%']);
    $talepler = $stmt->fetchAll();
} else {
    $talepler = $pdo->query("SELECT * FROM nikah_randevu_talepleri ORDER BY gonderim_tarihi DESC")->fetchAll();
}
...
$sayfaBasligi = 'Nikah Randevu Talepleri';
$aktifMenu = 'nikah-talepler';
include 'includes/admin-baslangic.php';
?>

<h4 class="fw-bold mb-3">Nikah Randevu Talepleri</h4>
            ...
            <tbody>
                ...
                <?php foreach ($talepler as $t): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($t['gelin_damat_adi']); ?></td>
                        <td><?php echo htmlspecialchars($t['esinin_adi']); ?></td>
                        <td><?php echo htmlspecialchars($t['telefon']); ?></td>
                        <td><?php echo htmlspecialchars($t['istenen_tarih']); ?></td>
                        <td style="max-width:220px;"><?php echo nl2br(htmlspecialchars($t['notlar'] ?? '')); ?></td>
