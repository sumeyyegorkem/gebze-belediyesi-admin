...
if (isset($_GET['durum_guncelle'], $_GET['id'])) {
    ...
    if (in_array($yeniDurum, $izinliDurumlar, true) && $guncellenecekId > 0) {
        ...
        $kayitStmt = $pdo->prepare("SELECT ad_soyad FROM temizlik_talepleri WHERE id = :id");
        ...
        if ($kayitAdi !== false) {
            islemKaydet($pdo, 'Temizlik Talebi Durumunu ' . ($durumEtiketleriLog[$yeniDurum] ?? $yeniDurum) . ' Yaptı', $kayitAdi);
        }
    }
    header('Location: temizlik-talepler-yonet.php?basarili=1');
    exit;
}
...
if (isset($_GET['sil'])) {
    ...
    if ($silinecekId > 0) {
        $silinecekStmt = $pdo->prepare("SELECT ad_soyad FROM temizlik_talepleri WHERE id = :id");
        ...
        if ($silinecekAdi !== false) {
            islemKaydet($pdo, 'Temizlik Talebi Sildi', $silinecekAdi);
        }
    }
    header('Location: temizlik-talepler-yonet.php?basarili=1');
    exit;
...
$arama = trim($_GET['q'] ?? '');
if ($arama !== '') {
    $stmt = $pdo->prepare("SELECT * FROM temizlik_talepleri WHERE ad_soyad LIKE :q1 OR mahalle LIKE :q2 OR konu LIKE :q3 ORDER BY gonderim_tarihi DESC");
...
$sayfaBasligi = 'Temizlik Talepleri';
$aktifMenu = 'temizlik-talepler';
