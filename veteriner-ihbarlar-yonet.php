...
if (isset($_GET['durum_guncelle'], $_GET['id'])) {
    ...
    if (in_array($yeniDurum, $izinliDurumlar, true) && $guncellenecekId > 0) {
        ...
        $kayitStmt = $pdo->prepare("SELECT ad_soyad FROM veteriner_ihbarlari WHERE id = :id");
        $kayitStmt->execute(['id' => $guncellenecekId]);
        $kayitAdi = $kayitStmt->fetchColumn();

        $guncelle = $pdo->prepare("UPDATE veteriner_ihbarlari SET durum = :durum WHERE id = :id");
        $guncelle->execute(['durum' => $yeniDurum, 'id' => $guncellenecekId]);

        if ($kayitAdi !== false) {
            islemKaydet($pdo, 'Veteriner İhbarı Durumunu ' . ($durumEtiketleriLog[$yeniDurum] ?? $yeniDurum) . ' Yaptı', $kayitAdi ?: 'Anonim');
        }
    }
    header('Location: veteriner-ihbarlar-yonet.php?basarili=1');
    exit;
}
...
if (isset($_GET['sil'])) {
    ...
    if ($silinecekId > 0) {
        $silinecekStmt = $pdo->prepare("SELECT ad_soyad FROM veteriner_ihbarlari WHERE id = :id");
        $silinecekStmt->execute(['id' => $silinecekId]);
        $silinecekAdi = $silinecekStmt->fetchColumn();

        $sil = $pdo->prepare("DELETE FROM veteriner_ihbarlari WHERE id = :id");
        $sil->execute(['id' => $silinecekId]);

        if ($silinecekAdi !== false) {
            islemKaydet($pdo, 'Veteriner İhbarı Sildi', $silinecekAdi ?: 'Anonim');
        }
    }
    header('Location: veteriner-ihbarlar-yonet.php?basarili=1');
    exit;
...
$arama = trim($_GET['q'] ?? '');
if ($arama !== '') {
    $stmt = $pdo->prepare("SELECT * FROM veteriner_ihbarlari WHERE ad_soyad LIKE :q1 OR konum LIKE :q2 OR konu LIKE :q3 ORDER BY gonderim_tarihi DESC");
    $stmt->execute(['q1' => '%' . $arama . '%', 'q2' => '%' . $arama . '%', 'q3' => '%' . $arama . '%']);
    $ihbarlar = $stmt->fetchAll();
} else {
    $ihbarlar = $pdo->query("SELECT * FROM veteriner_ihbarlari ORDER BY gonderim_tarihi DESC")->fetchAll();
}
...
$sayfaBasligi = 'Veteriner İhbarları';
$aktifMenu = 'veteriner-ihbarlar';
include 'includes/admin-baslangic.php';
?>

<h4 class="fw-bold mb-3">Veteriner İhbarları</h4>
            ...
            <tbody>
                ...
                <?php foreach ($ihbarlar as $t): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($t['konum']); ?></td>
